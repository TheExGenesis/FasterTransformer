...
def translate(args_dict):
    ...
    if 'mwhisper' not in model_path:
        hf_whisper_model = WhisperForConditionalGeneration.from_pretrained(model_path)
        tokenizer = WhisperTokenizer.from_pretrained(model_path)
        layernorm_type = "post_layernorm"
    else:
        hf_whisper_model = MWhisperForConditionalGeneration.from_pretrained(model_path)
        tokenizer = MWhisperTokenizer.from_pretrained(model_path)
        layernorm_type = "pre_layernorm"
    is_mwhisper = hf_whisper_model.config.add_final_layer_norm
    hf_whisper_model = hf_whisper_model.eval().to('cuda')
    try:
        fast_tokenizer = PreTrainedTokenizerFast.from_pretrained(model_path)
    except:
        fast_tokenizer = tokenizer

    config = hf_whisper_model.config
    ...
    if time_args.find("0") != -1 or time_args.find("2") != -1:
        hf_whisper_model = hf_whisper_model.to(rank)
        if args_dict['inference_data_type'] == 'fp16':
            hf_whisper_model = hf_whisper_model.half()
        elif args_dict['inference_data_type'] == 'bf16':
            hf_whisper_model = hf_whisper_model ## bfloat inference not supported yet
    ...
    if time_args.find("1") != -1 or time_args.find("3") != -1:
        ...
        ft_encoder_weight.load_from_model(hf_whisper_model.float())
        ...
        ft_decoding_weight.load_from_model(hf_whisper_model.float())
        if args_dict['inference_data_type'] == "fp16":
        ...
        while prev < len(src_text):
            ...
            if translation_result_list[i].frame_work == "HF":
                if translation_result_list[i].name.find("beamsearch") != -1:
                    hf_outputs = hf_whisper_model.generate(input_token.input_ids.to("cuda"), 
                ...
                if translation_result_list[i].name.find("sampling") != -1:
                    tmp_beam_size = 1
                return_dict = ft_whisper(input_token['input_ids'],
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lib_path', '--lib_path', type=str, default="lib/libth_whisper.so", metavar='STRING',
                        help='the path of FasterTransformer pytorch whisper op library.')
    parser.add_argument('-model_path', '--model_path', type=str, default=None, metavar='STRING',
                        help='T5 model path.')
    parser.add_argument('-model', '--model', type=str, default="whisper-base", metavar='STRING',
